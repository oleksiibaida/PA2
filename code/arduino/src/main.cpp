...
// Define MQTT-Topics und Nachrichten
  ...
  if (key)
  {
    ...
    case '#':
      ...
      if (passCorrect)
      {
        ...
        {
          send_mqtt_message(TOPIC_ALARM, PIN_CHANGE_STOP);
        ...
        else
        {
          ...
          send_mqtt_message(TOPIC_ALARM, PIN_CORRECT);
        }
        for (int i = 0; i < passLength; i++) // passInput leeren
          passInput[i] = -1;
      }
      ...
      else
      {
        ...
        send_mqtt_message(TOPIC_ALARM, PIN_WRONG);
        changePas = false;
        pinChangeTimeCounter = 0;
        for (int i = 0; i < passLength; i++)
          passInput[i] = -1; // passInput leeren
    ...
    case '*':
      ...
      if (!changePas)
      {
        ...
        send_mqtt_message(TOPIC_ALARM, PIN_CHANGE_START);
        changePas = true;
      }
      else if (changePas && checkOldPin)
      {
        ...
        if (inputFull)
        {
          ...
          send_mqtt_message(TOPIC_ALARM, PIN_CHANGE_SUC);
        }
        pinChangeTimeCounter = 0;
        changePas = false;
        checkOldPin = false;
